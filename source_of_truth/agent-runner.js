...
app.post('/webhook/pre-merge', (req, res) => {
  ...
  try {
    ...
    // Use subfolder for pre-merge logs
    const premergeMemoryPath = path.join(MEMORY_PATH, 'pre-merge');
    ...
    const prTitle = `🧠 Agent Pre-Merge Log: ${filename}`;
    const prBody = `This PR was auto-generated by **${AGENT_NAME}** to confirm agent liveness during pre-merge.
...
`;
    ...
    res.json({ ack: true, schemaVersion, branch });
  } catch (err) {
    console.error('❌ Pre-merge error:', err.message);
